...
class PetAdoptionTablePlugin {
  function __construct() {
    ...
    // 'activate_(name of plugin folder)/(name of main php file)' hook - only run when plugin gets activated(one time)
    add_action('activate_new-database-table/new-database-table.php', array($this, 'onActivate'));
    // run when we reload or refresh the admin side
  ...
  function onActivate() {
    ...
    // create numeric ID number for each pet
    // tell SQL which character set we're using at the end
